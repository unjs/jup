...
import { canonicalEnvName, ENV, envEntry, readEnv } from "../config/env-vars.ts";
...
/**
 * §15.4's precedence: `JUP_CAFILE` (or its legacy `COREPACK_` spelling), then
 * `.npmrc`'s `cafile`/`ca`, then the platform trust store.
 *
 ...
 */
export function readCaBundle(
  path: string,
  source: string = canonicalEnvName(ENV.CAFILE),
): string[] {
...
function trustStoreFor(settings: TlsSettings): string[] | undefined {
  if (settings.cafile !== undefined) {
    return readCaBundle(settings.cafile, settings.cafileSource ?? canonicalEnvName(ENV.CAFILE));
  }
