...
* Each test runs with a **fresh `COREPACK_HOME`** and a clean environment with every
  `COREPACK_*`, `JUP_*`, `DEBUG`, and `FORCE_COLOR` variable removed.
* Rows 1–147 assert corepack's verbatim output and are therefore run through the
  **`corepack` entry point** (§17.4 R12), whose usage lines and tool-name spellings
  are corepack's. Running them under `jup` differs only in those spellings. Rows
  148–207 are §15.38's; rows 208–233 are §17.9's, which amends this list twice: its
  store-home rows set those variables themselves, and its role-sensitive rows need
  the test-only table fixture §17.9 describes.
