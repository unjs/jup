...
to be set. Both are quirks of the fallback chain rather than design; a conforming
implementation MUST reproduce them for compatibility with existing caches.
...
`<name>/<version>/` subtrees, `.corepack` markers included. It is literally a copy of
cache subtrees, not a repackaging.

Default output path: `./corepack.tgz`. `-o/--output` overrides. `--json` prints
...
```
scan entries; consider only those whose LAST path segment is ".corepack"
    fewer than 3 segments (i.e. not <name>/<version>/.corepack) → hasShortEntries
