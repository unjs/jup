...
Note the order: `XDG_CACHE_HOME` is consulted **before** `LOCALAPPDATA` on every
platform, including Windows, and `LOCALAPPDATA` is consulted on POSIX if it happens
to be set. Both are quirks of the fallback chain rather than design.

> **§15.13 point 5 narrows this**: `LOCALAPPDATA` MUST be consulted **only on
> Windows**. Since §17.6 C2 moves the last segment off `node/corepack`, the chain is
> no longer shared with corepack and there is nothing left for the quirk to be
> compatible with — so §15.13's chain is the one to implement, and §15.13's claim to
> be "the one place this spec breaks store-location compatibility" is superseded by
> C2, which breaks it wholesale and deliberately.

> **Divergence (§17.6 C2).** Corepack's last path segment is `node/corepack`. This
> spec's is **`jup`**. Two reasons, both about ownership: a directory named after
> another program is the wrong place to write artifacts that program never created
> (a runtime distribution, §17.2), and a real corepack's `cache clean` — which is
> `rm -rf` on that path (§07.9) — would delete them. `COREPACK_HOME` remains
> honoured, so a user who has already pointed the cache somewhere keeps it, and
> `JUP_HOME` wins when both are set (§11.6). No migration is performed (§17.6 C8):
> the cost of the move is one re-download per tool per machine.
...
└── v1/
    ├── corepack-<pid>-<8 hex>/      # transient; temp dirs live here (same FS)
    ├── npm/
    │   └── 11.14.1/
    │       ├── .corepack
...
The marker file is named **`.jup`**. A reader MUST also accept a directory whose only
marker is `.corepack`, so that a store inherited from corepack — or from an older
`COREPACK_HOME` still in use — is warm rather than re-downloaded; a writer MUST
always write `.jup` (§17.6 C3). The two files have identical contents.

The cost is one extra `stat` per probe, and only on a store this tool did not write:
`.jup` is tried first, so a store jup installed pays nothing, while a corepack store
pays a miss on every warm run until its entries are reinstalled. §01.3's fast-path
budget is amended to "one marker read, or two on an inherited store". Everywhere
below, `.corepack` names either file — **except in §07.10**, where the archive
validator scans path segments as literal strings and the accepted set has to be
spelled out.
...
```
try  read <installFolder>/<name>/<version>/.corepack, parse
...
```
tmp := <installFolder>/corepack-<pid>-<random hex>      # MUST be on the same filesystem
stream := GET url                                        # §05
...
`<name>/<version>/` subtrees, markers included. It is literally a copy of cache
subtrees, not a repackaging — so the marker it carries is whatever the store holds,
`.jup` for anything jup installed and `.corepack` for an inherited entry (§17.6 C3).

Default output path: **`./jup.tgz`** (§17.6 C9; corepack's was `./corepack.tgz`). `-o/--output` overrides. `--json` prints
...
```
scan entries; consider only those whose LAST path segment is ".jup" OR ".corepack"
    fewer than 3 segments (i.e. not <name>/<version>/<marker>) → hasShortEntries
    otherwise record (segments[0], segments[1]) as (name, reference)
if hasShortEntries or no entries recorded:
    → UsageError `Invalid archive format; did it get generated by 'corepack pack'?`
...
Accepting both markers is what makes the archive portable in both directions: an
archive corepack produced installs here, and an archive jup produced from an inherited
store is not rejected as `Invalid archive format` for carrying the older name. An
archive jup produced from its own store carries `.jup` and is **not** installable by
corepack; that is a one-way break, and the reason `pack`'s output is a
version-manager artifact rather than an interchange format.
