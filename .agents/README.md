...
This directory defines how jup discovers, downloads, verifies, caches, and runs package managers. The specification is independent of any programming language or runtime so that the same behavior can be implemented in JavaScript, Rust, Zig, Go, C, or another suitable language.
...
| **Focused** | Manage and run package manager versions, and nothing more. | Do not add plugins, a configuration language, or telemetry. The commands in §09 are the complete public CLI. |
