...
| `JUP_DEFAULT_TTL` | hours, default `24` | How long a recorded global default stands before it is re-checked; `0` never expires, and `cache install -g` / `pack` pins are exempt (§04.6) | yes |
...
| `JUP_ENABLE_PM_LOCKFILE` | `0` | Stop reading the package manager's own lockfile: both its committed resolution (§04.4) and the major its format implies (§04.6) | yes |
