...
An existing `packageManager` is still read, and jup keeps it up to date whenever
it writes a pin — so a project that already has one stays readable by Corepack,
npm and Yarn. A project that has none does not gain one, and those tools will
not see the pin. Add a `packageManager` field by hand if you need them to.
