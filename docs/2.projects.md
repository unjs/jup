...
jup use pnpm@12          # keep this major range and create jup.lock
jup use pnpm@^12         # keep this caret range and create jup.lock
jup use pnpm@latest      # resolve the tag now, then write an exact pin
```

Partial versions such as `12` and `12.1` are ranges. jup keeps a range as typed
