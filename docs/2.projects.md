...
That file is committed, so jup reads it. When a range has no `jup.lock` answer,
...
- `download` — continue silently, because jup has already downloaded and run the
  version this object declares. Where jup cannot do that — the object is
  malformed, or names no version, or its digest is unusable or disagrees with
  the one on `packageManager` — it warns instead.
...
package manager — `packageManager` or `devEngines.packageManager` — and `jup use
bun@1.4.0` writes that pin.
