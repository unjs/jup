...
## Pin aube, Bun, or Deno

aube, Bun, and Deno are pinned like any other package manager:
...
One difference is visible in the file: there is **no digest suffix**. These three ship a separate compiled binary for each operating system and processor, so `bun@1.4.0` is six different downloads. A digest recorded in a committed file would describe whichever machine wrote it, and would fail for everybody else with a hash mismatch.
...
aube is shimmed by a bare `jup enable` like npm, pnpm, and Yarn. Bun and Deno are not, because those names usually belong to a runtime you installed yourself; see [Getting started](./getting-started).
