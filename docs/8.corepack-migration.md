...
- `.corepack.env`, when there is no `.jup.env` beside it;
- `COREPACK_*` environment variables;
- npm, pnpm, and Yarn command names; and
- a warm Corepack cache entry, which it recognises by its `.corepack` marker.

Every `COREPACK_*` variable also has a `JUP_*` spelling. If both are set, `JUP_*` takes priority. For the variables Corepack itself has, the two spellings are permanent equals; for the ones jup adds, `JUP_*` is the real name and `COREPACK_*` is accepted for convenience.
...
jup writes its own names and reads Corepack's. Nothing is migrated or moved:

| Corepack | jup | Reading Corepack's name |
| --- | --- | --- |
| `.corepack.env` | `.jup.env` | yes, when `.jup.env` is absent |
| `.corepack` cache marker | `.jup` | yes |
| `corepack.tgz` (`pack` output) | `jup.tgz` | `-o` overrides either; `install -g` accepts both |
| `<cache>/node/corepack` | `<cache>/jup` | `COREPACK_HOME` is still honoured |
| `%LOCALAPPDATA%\node\corepack\bin` | `%LOCALAPPDATA%\jup\bin` | `jup disable` restores from its own record |

Moving to the new cache directory costs one download per package manager. There is no migration step and nothing to clean up: an unused Corepack cache can simply be deleted.

`.jup.lock` is jup's own file. If a project already has a `.corepack.lock` from an earlier jup, it keeps working; the next time jup records a resolution it writes `.jup.lock` and removes the old file, which shows up as a rename in `git status`.
...
- `.jup.env` cannot disable verification, add trust keys, enable unsafe URLs, or provide registry credentials.
