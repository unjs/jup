...
Corepack manages npm, pnpm, and Yarn. jup adds **aube**, **Bun**, and **Deno**, which Corepack cannot run at all: its design assumes a package manager is portable JavaScript, and all three ship a compiled binary per platform.
...
- **No shim by default, for the two runtimes.** `jup enable` does not claim the `bun`, `bunx`, or `deno` names, because those usually belong to an installation you chose. Run `jup enable bun deno` to opt in. aube is a package manager rather than a runtime, so `aube`, `aubr`, and `aubx` are shimmed like `pnpm` and `pnpx`.

A manifest pinning aube, Bun, or Deno is not readable by Corepack, so this is the one direction the compatibility above does not run in.
...
`JUP_REGISTRY_NPM`, `JUP_REGISTRY_PNPM`, `JUP_REGISTRY_YARN`, `JUP_REGISTRY_AUBE`, `JUP_REGISTRY_BUN`, and `JUP_REGISTRY_DENO` let you mirror one manager without changing the registry used by the others.
...
jup enables npm, pnpm, Yarn, and aube by default; Bun and Deno are supported but must be requested by name (`jup enable bun deno`). If a migration behaves differently than expected, run `jup info` to inspect the selected manifest, lockfile, registry, cache entry, and shim path, then continue with [Troubleshooting](./troubleshooting).
