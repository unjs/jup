...
Corepack manages npm, pnpm, and Yarn. jup adds **Bun** and **Deno**, which Corepack cannot run at all: its design assumes a package manager is portable JavaScript, and both ship a compiled binary per platform.
...
- **No shim by default.** `jup enable` does not claim the `bun`, `bunx`, or `deno` names, because those usually belong to an installation you chose. Run `jup enable bun deno` to opt in.

A manifest pinning Bun or Deno is not readable by Corepack, so this is the one direction the compatibility above does not run in.
...
`JUP_REGISTRY_NPM`, `JUP_REGISTRY_PNPM`, `JUP_REGISTRY_YARN`, `JUP_REGISTRY_BUN`, and `JUP_REGISTRY_DENO` let you mirror one manager without changing the registry used by the others.
...
jup enables npm, pnpm, and Yarn by default; Bun and Deno are supported but must be requested by name (`jup enable bun deno`). If a migration behaves differently than expected, run `jup info` to inspect the selected manifest, lockfile, registry, cache entry, and shim path, then continue with [Troubleshooting](./troubleshooting).
