...
After `jup enable`, run npm, pnpm, Yarn, or aube as usual (and Bun, Deno, or nub after `jup enable bun deno nub`):
...
If `packageManager` itself contains a range, `up` keeps the range in the manifest and refreshes its exact resolution in `.jup.lock`.
