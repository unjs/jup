...
Bun and Deno publish a separate compiled binary for each platform. jup downloads the per-platform npm package for your machine — `@oven/bun-linux-x64`, `@deno/darwin-arm64`, and so on — rather than the small `bun` or `deno` package, which only exists to fetch that binary with an install script.
