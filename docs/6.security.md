...
## aube, Bun, Deno, and nub

aube, Bun, Deno, and nub publish a separate compiled binary for each platform. jup downloads the per-platform npm package for your machine — `@endevco/aube-linux-x64`, `@oven/bun-linux-x64`, `@deno/darwin-arm64`, `@nubjs/nub-linux-x64`, and so on — rather than the small `@endevco/aube`, `bun`, `deno`, or `@nubjs/nub` package, which only exists to fetch that binary with an install script.
...
Because jup fetches the platform package directly, none of these managers' install scripts run. That is the point: the `preinstall` or `postinstall` step in the small wrapper package exists to shell out to `npm install` or to chmod what it fetched, and jup does the fetch itself, verified, with the executable bit taken from the archive.
