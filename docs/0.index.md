...
project. The range stays in `package.json`.

To record the selected release in a committed `jup.lock` as well:
...
independently — unless the package manager keeps its own committed record, as
pnpm 12 does, which jup reads. When `node_modules` already exists, jup may reuse
that registry answer for 24 hours from the disposable `node_modules/.jup/jup.lock`
cache. For
...
Or ask for every supported tool at once, which stays correct as the list grows:
...
::warning
`jup enable node` — and `jup enable --all`, which includes it — can put jup's
`node` ahead of your current Node.js for every shell command. You do not need
this shim to run a selected Node version. Use `jup node@^22 script.js` instead.
::
...
2. [Set up GitHub Actions](/actions)
3. [Set up CI and offline builds](/ci)
4. [Configure registries and authentication](/registry)
5. [Read the command reference](/commands)
