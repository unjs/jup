...
Bun and Deno are supported but are **not** shimmed by default, because those names are runtimes and usually belong to an installation you chose yourself. (aube is a package manager, so it is shimmed by default like the rest.) Ask for them by name:
